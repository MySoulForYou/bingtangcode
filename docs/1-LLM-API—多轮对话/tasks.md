...
- 在命令行应用中，异步线程是为了处理耗时的网络请求而不阻塞其他后台工作，
  倒计时器（挂起主线程）是为了防止主线程跑到前面打印提示符导致屏幕输出错乱（实现回合制交互），而将其设为守护线程并配合手动断开则是为了提供双重保险：
  既允许用户中途打断耗时任务立刻重置状态，又能确保主程序崩溃或正常退出时，不会留下僵尸进程在后台偷偷耗费资源。
## 1. 项目骨架搭建
